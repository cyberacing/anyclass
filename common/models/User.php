...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function scenarios() : array
    {
        ...
        $scenarios[self::SCENARIO_ADMIN] = array_keys($this->getAttributes());
        return $scenarios;
    }
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($insert) {
            $userRole = self::CLIENT_ROLE;
            if ($this->scenario == self::SCENARIO_ADMIN) {
                $userRole = self::ADMIN_ROLE;
            }
            ...
            if ($role === null) {
                throw new RuntimeException("Не найдена роль: {$userRole}");
            }
